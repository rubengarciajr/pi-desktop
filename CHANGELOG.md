...
### Changed
- **Pi SDK 0.80.10 → 0.82.1.** Two minor bumps (0.81.x + 0.82.x), so `^0.80.10` would not have picked either up. Highlights: **Claude Opus 5** support (Anthropic + Amazon Bedrock, adaptive thinking incl. `xhigh`, prompt caching); new built-in **Qwen Token Plan**, **OpenRouter**, and **Kimi Code** sign-in providers; **llama.cpp** router support with Hugging Face model search/download; and full provider registration for extensions. Compaction and branch summarization now follow the configured retry policy instead of failing on a transient provider error; model catalogs revalidate with ETag/`If-None-Match`; DNS failures (`ENOTFOUND`/`EAI_AGAIN`) now trigger automatic retries; and GPT-5.6 Codex defaults to a 272K context window (avoids automatic long-context pricing).

...
### Fixed
- **Worktree sessions now stay inside their own checkout.** Inherited from the Pi SDK: the `bash`, `edit`, `find`, `grep`, `ls`, `read`, and `write` tools were ignoring `ctx.cwd`, so an agent could read and write against the process working directory instead of the session's own folder. That undercut v0.7.0's worktree sessions, whose whole premise is a per-session isolated checkout — an agent working in a worktree could touch files outside it. ([earendil-works/pi#8627](https://github.com/earendil-works/pi/pull/8627))

### Changed
- **Pi SDK 0.84.3 → 0.84.4.** A maintenance bump with no breaking changes; no app-code changes were required. Carries the `ctx.cwd` fix above, plus `ui_prompt_start`/`ui_prompt_end` extension events, RPC `clear_queue`, terminal capability overrides, and an experimental DeepSeek V4 Flash Vision model.
...
- **Migrated `ModelRuntime.reloadConfig()` → `refresh({ allowNetwork: false })`.** 0.82 removed `reloadConfig()`; `refresh()` is its successor. Pi Desktop calls this in `refreshModelRegistry()` after a custom model is added/edited/removed. We pass `allowNetwork: false` so it stays a local config reload (matching the old no-network behavior) rather than defaulting to the runtime's network setting and firing catalog fetches on every edit — mirroring the SDK's own internal "config changed" calls.
- Every other SDK entry point Pi Desktop uses is unchanged (`createAgentSessionServices` / `createAgentSessionFromServices` / `createAgentSessionRuntime`, and the `ModelRuntime` auth/model methods behind MOA, Tag Team, and the settings panel). 0.81.0 altered the agent-core streaming API but 0.81.1 restored the pre-0.81 fallback.
- **0.84.0's breaking changes were checked and do not affect Pi Desktop.** Its `message_update` change (dropping the cumulative `message` and `assistantMessageEvent.partial` fields) applies to *serialized JSON/RPC* output; Pi Desktop consumes the in-process `AgentSessionEvent` stream, which still carries both — and our handler already reads deltas only (`applyDelta` handles `text_delta` / `thinking_delta` / `toolcall_*`), which is the model 0.84 now mandates. `setRuntimeApiKey()`'s changed third parameter is optional and we pass only two arguments.
- **Security posture:** the bump moved the SDK's nested `protobufjs` 7.5.0 → 7.6.5, clearing advisory GHSA-j3f2-48v5-ccww.
